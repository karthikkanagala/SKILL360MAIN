...
COMMON_SKILLS = [
    # Programming Languages (excluding R and C - handled separately)
    'python', 'java', 'c++', 'c#', 'go', 'rust', 'typescript', 'javascript', 'swift', 'kotlin', 'ruby', 'php', 'perl', 'objective-c', 'matlab', 'dart', 'scala', 'groovy', 'lua', 'visual basic', 'assembly', 'fortran', 'cobol', 'delphi', 'abap', 'sas',
    ...
    # Data & ML
    'pandas', 'numpy', 'scipy', 'scikit-learn', 'sklearn', 'tensorflow', 'pytorch', 'torch', 'keras', 'xgboost', 'catboost', 'lightgbm', 'mlops', 'machine learning', 'deep learning', 'nlp', 'natural language processing', 'computer vision', 'data analysis', 'data science', 'data engineering', 'big data', 'analytics', 'artificial intelligence',
    # Databases
    ...
    # Cloud & DevOps
    'aws', 'amazon web services', 'azure', 'microsoft azure', 'gcp', 'google cloud', 'cloud', 'docker', 'kubernetes', 'k8s', 'terraform', 'ansible', 'ci/cd', 'cicd', 'jenkins', 'airflow', 'bash', 'shell', 'powershell', 'vagrant', 'openshift', 'cloudformation', 'circleci', 'travis ci', 'github actions', 'gitlab ci', 'argo', 'helm', 'prometheus', 'grafana', 'datadog', 'new relic', 'splunk', 'puppet', 'chef', 'saltstack', 'consul', 'nomad', 'vercel', 'netlify', 'heroku',
    # Tools & Libraries
    'git', 'github', 'gitlab', 'bitbucket', 'jira', 'confluence', 'notion', 'slack', 'excel', 'powerbi', 'power bi', 'tableau', 'looker', 'matplotlib', 'seaborn', 'plotly', 'bokeh', 'd3.js', 'highcharts', 'chart.js', 'sqlalchemy', 'orm', 'pypdf2', 'docx', 'openpyxl', 'beautifulsoup', 'scrapy', 'requests', 'httpx', 'aiohttp', 'pytest', 'unittest', 'mocha', 'jest', 'junit', 'testing', 'selenium', 'cypress', 'playwright', 'puppeteer', 'postman', 'swagger', 'openapi', 'soapui', 'vs code', 'vscode', 'visual studio', 'intellij', 'pycharm', 'eclipse', 'vim', 'emacs',
    # LLM & AI
    'prompt engineering', 'langchain', 'openai', 'chatgpt', 'gpt', 'llm', 'large language model', 'huggingface', 'hugging face', 'transformers', 'bert', 'gpt-3', 'gpt-4', 'llama', 'gemini', 'claude', 'anthropic', 'rag', 'vector database', 'pinecone', 'weaviate', 'chromadb',
    # Mobile
    'android', 'ios', 'flutter', 'react native', 'xamarin', 'ionic', 'cordova', 'mobile development', 'mobile app',
    # Other
    'agile', 'scrum', 'kanban', 'linux', 'unix', 'windows', 'macos', 'waterfall', 'etl', 'project management', 'leadership', 'communication', 'problem solving', 'testing', 'unit testing', 'integration testing', 'tdd', 'bdd', 'oop', 'object oriented', 'functional programming', 'soa', 'design patterns', 'system design', 'architecture', 'software architecture', 'ux', 'ui', 'ux/ui', 'ui/ux', 'user experience', 'user interface', 'figma', 'sketch', 'adobe xd', 'photoshop', 'illustrator', 'accessibility', 'security', 'cybersecurity', 'networking', 'tcp/ip', 'http', 'https', 'ssl', 'tls', 'oauth', 'jwt', 'authentication', 'authorization',
]

# R language context patterns - only match R when it's clearly the programming language
R_PATTERNS = [
    ...
    r'\bR\s+for\s+data\b',
    r'\bR\s+shiny\b',
    r'\bshiny\b',      # R-specific framework
    r'\bggplot\b',     # R-specific library
    r'\bggplot2\b',    # R-specific library
    r'\bdplyr\b',      # R-specific library
    r'\btidyverse\b',  # R-specific ecosystem
    r'\btidyr\b',      # R-specific library
    r'\bR\s*,\s*Python\b',  # "R, Python" pattern
    r'\bPython\s*,\s*R\b',  # "Python, R" pattern
    r'\bR\s+and\s+Python\b',
    r'\bPython\s+and\s+R\b',
    r'\bR\s*/\s*Python\b',
    r'\bCRAN\b',       # R package repository
]

# C language context patterns (but not C++, C#)
C_PATTERNS = [
    r'\bC\s+programming\b',
    r'\bC\s+language\b',
    r'\bANSI\s+C\b',
    r'\bC\s*,\s*(?![\+#])',  # "C, Python" but not "C++" or "C#"
    r',\s*C\s*(?![+#])',    # "Python, C" but not "C++" or "C#"
    r'\bC\s+and\s+',
    r'\sand\s+C\b(?!\s*[+#])',
    r'\bpure\s+C\b',
...
def extract_skills(text, skills=None):
    ...
    # Check for R language with context patterns
    for pattern in R_PATTERNS:
        if re.search(pattern, text, re.IGNORECASE):
            found.add('R')
            break
    
    # Check for C language with context patterns
    for pattern in C_PATTERNS:
        if re.search(pattern, text, re.IGNORECASE):
            found.add('C')
            break
    
    # Word boundary matching for other skills
    for skill in skills:
        # Create regex pattern with word boundaries
        escaped_skill = re.escape(skill.lower())
        
        # Handle C++ and C# specially (no trailing boundary needed)
...
def get_skill_categories():
    ...
    return {
        "Programming Languages": ['Python', 'Java', 'JavaScript', 'TypeScript', 'C++', 'C#', 'Go', 'Rust', 'Ruby', 'PHP', 'Swift', 'Kotlin', 'R', 'Scala', 'Dart', 'C'],
